...
运行：  python 05_rag_agent.py        （默认只展示检索→证据拼装）
        python 05_rag_agent.py --chat （进入交互对话：OpenAI 或本机 Ollama）
...
THRESHOLD = 0.5  # voyage-4-lite 校准值：相关约 0.33–0.42，不相关约 0.77
...
if __name__ == "__main__":
    ...
    banner()
    if "--chat" in sys.argv:
        # 共用的对话后端选择器：有 OpenAI Key 就用云端，否则自动连
        # 本机 Ollama。Voyage 只负责 embedding，不是聊天模型。
