...
前置：  pip install -r requirements.txt
       真 embedding：在仓库根目录 VOYAGE_API_KEY.txt 填一次 Key，全课程 Demo 共用。
...
banner()  # 打印这次实际使用的 embedding 后端

# Voyage 会分别优化“检索问题”和“候选文档”的投影方式。
q_vec = embed([query], input_type="query")[0]
cand_vecs = embed(candidates, input_type="document")
